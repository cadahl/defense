namespace Client.Sim
{
	...
	//	public delegate void AddObjectPacketReceiver(AddObjectPacket aop);
	//	public delegate void RemoveObjectPacketReceiver(int uid);
		public delegate void UpdateHandler(StateUpdate update);
		
//		public event AddObjectPacketReceiver ObjectAdded; 
		//public event RemoveObjectPacketReceiver ObjectRemoved; 
			...
			lock(_objectAddList)
			{
				foreach(GameObject obj in _objectAddList)
				{
					if (obj != null) 
					{
						...
						/*	var oa = ObjectAdded;
							if(oa != null) 
								oa(new AddObjectPacket(obj.Uid, obj.TypeId));
						*/}
					}
				}
		...
		protected void UpdateObjects(long updateCount)
		{
			...
			su.Timestamp = DateTime.Now.Ticks;
			su.UpdatedObjects = new SortedDictionary<int,ObjectStatePacket>();
				
			var state = new List<ObjectStatePacket>();
			...
			lock(_objects)
			{
				...
				{
					_spatialHash.Remove(obj);
					su.UpdatedObjects.Add(obj.Uid, obj.Update (updateCount));
					_spatialHash.Add(obj);
				}
			}							
